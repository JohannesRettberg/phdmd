...
# method index from [POD, OI, PHDMD] - [0,1,2] for all methods
num_all_methods = 3
...
for i_pod, use_energy_weighted_POD in enumerate(use_energy_weighted_PODs):
    ...
    for i_noise, noise in enumerate(noise_levels):
        ...
        npzdata = np.load(os.path.join(config.evaluations_path, f'{experiment_name_i}_{pod_type}.npz'))
        ...
        if noise is not None:
            labels = np.core.defchararray.add(labels, f' ($s={noise_levels[i_noise]:.0e}$)')

        if iteration_num == 0:
            h2_norms_all = h2_norms[:,method_idx]
            labels_all = labels[method_idx]
        else:
            h2_norms_all = np.concatenate((h2_norms_all,h2_norms[:,method_idx]),axis=1)
...
markevery = 10
plot(reduced_orders, h2_norms_all.T[:, np.newaxis, :], label=labels_all,
        c=c_all, ls=ls, marker=markers, markevery=markevery,
        yscale='log', ylabel='$\mathcal{H}_2$ error', grid=True, subplots=False,
        xlabel='Reduced order', fraction=1, name=plot_name)